...
- 用中文沟通，改文件前先说明要改什么。
- 网站内容、样式或资产文件改完后，默认执行验证、commit 并 push 到远端 `main`，让 GitHub Pages 线上页面同步更新。
- push 前先检查 `git diff` / `git status`，确认只包含当前任务相关改动。
...
- push 后检查 GitHub Pages 状态和线上地址：`https://linshenghe.github.io/`。
